...
pub fn execute(
    ...
    let (output_path, is_temp) = if temp {
        let temp_dir =
            std::env::temp_dir().join(format!("vibe-graph-architect-{}", uuid::Uuid::new_v4()));
        (temp_dir, true)
    } else {
    ...
    let logical_graph: SourceCodeGraph =
        serde_json::from_str(&content).context("Failed to parse input graph")?;

    println!(
        "📊 Loaded graph with {} nodes and {} edges",
        logical_graph.node_count(),
        logical_graph.edge_count()
    );
    ...
    if dry_run {
        ...
        if is_temp {
            println!(
                "\n(Would write to temporary directory: {})",
                output_path.display()
            );
        }
        return Ok(());
    ...
    println!("\n💾 Materializing to {}...", output_path.display());
    ...
    for node in &proposed_graph.nodes {
        ...
        let path_str = node
            .metadata
            .get("path")
            .context("Node missing path metadata")?;
            ...
            vibe_graph_core::GraphNodeKind::File | vibe_graph_core::GraphNodeKind::Module => {
                ...
                if !path.exists() {
                    fs::write(
                        &path,
                        format!(
                            "// Generated by Vibe-Graph Architect\n// Node: {}\n",
                            node.name
                        ),
                    )?;
    ...
    if is_temp {
        println!("\n📍 Output available at: {}", output_path.display());
    }
